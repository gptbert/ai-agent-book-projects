...
**前五个训练步骤的实际观察**揭示了训练过程的动态特征和性能演化趋势。步骤一和步骤二的耗时高度一致，分别为 4853 秒和 4859 秒（均为 81 分钟），表明训练流程在初始阶段已经进入稳定状态。然而，从步骤三开始出现了耗时增长的现象，步骤三耗时 5431 秒（90.5 分钟），步骤四耗时 5620 秒（93.7 分钟），分别比前两步增加了 12% 和 16%。这种耗时增长主要源于响应长度的持续增加：平均响应长度从步骤一的 2707 tokens 增长到步骤五的 3310 tokens，累计增长了 22%。更多响应达到最大长度限制并被截断，截断比例从 0.85% 上升到 1.95%，相当于从 4-5 个响应增加到 10 个响应触及 16384 tokens 的上限。
...
**模型学习进展**在前五个步骤中呈现出明显的改善轨迹。训练集上的平均奖励从步骤一的 0.058 持续提升至步骤五的 0.205，对应的准确率从约 53% 提升到 60%，累计提升了 7 个百分点。然而，学习速度在逐步放缓：步骤一到二提升了 5 个百分点，步骤二到三提升了 1 个百分点，步骤三到四几乎没有提升，步骤四到五提升了约 2 个百分点。验证集上的表现显示，平均准确率（mean@30）为 27.9%，相比初始验证的 18.6% 提升了 9.3 个百分点。更值得关注的是 Best-of-30 指标达到 62.9%，说明模型在 30 次采样中至少能生成一次正确答案的能力很强，但生成一致性仍有待提高。多数投票准确率（maj@30）为 36.9%，相比初始验证的 27.8% 有显著提升，表明模型的整体可靠性在增强。

**响应生成模式的演化**呈现出复杂的特征。平均响应长度从 2707 tokens 增长到 3310 tokens，增幅达 22%，但平均交互轮数保持相对稳定（在 7.7-7.8 轮之间小幅波动），这意味着每轮交互产生的内容在增加，从步骤一的平均每轮 346 tokens 增长到步骤五的 422 tokens。最慢的单个响应生成时间从步骤一的 2492 秒（41.5 分钟）持续增长到步骤五的 3257 秒（54.3 分钟），增幅达 31%。这些超长响应都达到了 16384 tokens 的上限，它们的生成时间几乎完全决定了整个 Rollout 阶段的总耗时，体现了显著的木桶效应。整个训练过程处理的 token 总量也在持续增加，从步骤一的 24.9M tokens 增长到步骤五的 29.9M tokens，增幅达 20%。

**系统资源利用情况**在前五个步骤中保持了相对稳定的模式。GPU 显存使用稳定在 215 GB 左右，接近但未超过 8 张 H200 GPU 的总容量上限。CPU 内存使用在步骤二达到 227 GB 的峰值后回落，在步骤三到五之间在 220-227 GB 范围内波动，没有出现持续增长的趋势，这表明 CPU offload 机制的内存管理是健康的。模型的算力利用率（MFU）在 39.7%-40.1% 之间小幅波动，保持在相对稳定的水平。整体吞吐量从步骤二的 650 tokens/s 峰值逐渐下降到步骤五的 615 tokens/s，这种下降与响应长度的增加直接相关，反映了处理更长序列时的额外开销。

**训练内部技术指标**展现了 DAPO 算法的稳定运行状态和设计优势。策略梯度损失（actor/pg_loss）在前五个步骤中保持在 -0.0018 到 -0.0022 之间的小幅波动，绝对值很小且稳定，表明策略更新处于平稳状态。策略裁剪比例（actor/pg_clipfrac）从步骤一的 0.197% 逐渐下降到步骤五的 0.177%，始终保持在很低的水平，说明几乎所有的策略更新都在允许的范围内，没有触发过度更新的保护机制。这个低裁剪比例验证了 Clip-Higher 策略的有效性——通过不对称裁剪（0.2 到 0.28），为向上更新预留了足够空间，避免了过度裁剪导致的训练停滞。下界裁剪比例（actor/pg_clipfrac_lower）在所有步骤中都接近于零（最大仅为 1.9e-7），表明没有出现策略退化的情况，证明了严格限制向下更新的策略是成功的。

KL 散度（actor/ppo_kl）的演化轨迹反映了策略变化的幅度和 DAPO 算法的保守特性。步骤一到步骤五的 KL 散度分别为 1.93e-5、2.35e-5、1.56e-5、1.30e-5 和 1.51e-6，数值在 1e-6 到 2e-5 之间波动，始终保持在极小的范围内。特别值得注意的是步骤五的 KL 散度仅为 1.51e-6，比前几步降低了一个数量级，这表明在该步骤中策略更新极其保守，新策略与旧策略几乎完全一致。这种极小的 KL 散度配合较低的裁剪比例，共同保证了训练的稳定性，避免了策略崩溃或性能突然下降的风险。由于配置中设置了 kl_coef=0.0，KL 散度项并未直接加入损失函数，这些观察到的小 KL 值完全是裁剪机制和小学习率（1e-6）自然产生的结果，体现了 DAPO 算法内在的稳定性。

梯度范数（actor/grad_norm）在前五个步骤中呈现温和的下降趋势，从步骤一的 0.132 逐渐降至步骤五的 0.113，下降了约 14%。梯度范数的大小直接反映了参数更新的强度，其下降趋势说明模型参数在逐渐接近某个局部最优点，需要的调整幅度越来越小。所有步骤的梯度范数都保持在 0.11-0.13 的健康范围内，既没有出现梯度消失（过小）也没有梯度爆炸（过大），表明反向传播过程运行良好。策略熵（actor/entropy）保持在 0.154-0.159 之间，变化幅度很小，说明模型的探索-利用平衡保持稳定。这个稳定的熵值证明了 Clip-Higher 策略成功避免了熵崩溃问题——在使用对称裁剪的 GRPO 基线中，熵往往会在训练早期快速下降到接近零，导致模型失去探索能力。
