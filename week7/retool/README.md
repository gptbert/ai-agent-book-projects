...
**训练各阶段的实际耗时**揭示了计算资源的分配模式。Rollout 生成阶段耗时 2529 秒（42 分钟），这是最耗时的阶段，期间 vllm 引擎需要生成所有响应并与 SandboxFusion 进行频繁交互。Reference Model 的 log probability 计算耗时 525 秒（8.7 分钟），这个阶段对已生成的响应进行前向传播以获取旧策略下的概率值。策略更新阶段耗时 1795 秒（30 分钟），这个阶段将 512 个样本分成 8 个 mini-batch 依次进行梯度下降，由于启用了 FSDP 和 CPU offload，大量时间消耗在参数的 CPU-GPU 传输上。整个训练步骤的总耗时为 4853 秒（约 81 分钟），其中还包括奖励计算（可忽略不计）和其他辅助操作。
...
**第一个训练步骤的实际表现**提供了宝贵的性能基准。根据实际运行数据，完整的第一个训练步骤耗时 4853 秒（约 81 分钟），其中包含了初始验证阶段。训练过程的平均奖励从初始验证的 -0.464 提升到 0.058，表明模型在单个训练步骤后就开始展现学习效果。响应的平均长度为 2707 tokens，比初始验证阶段略短，说明模型开始学会更有效地使用工具。平均交互轮数从 6.59 增加到 7.82，显示模型在尝试更深入的推理过程。约 0.85% 的响应达到了最大长度限制（16384 tokens），这些超长响应主要出现在特别复杂的数学问题上。

**后续训练步骤的加速效应**会显著改善训练效率。由于第一个步骤已经完成了 vllm 引擎的预热、CUDA kernel 编译和各种初始化工作，后续步骤将避免这些一次性开销。更重要的是，验证频率设置为每 5 个步骤一次（test_freq=5），意味着大部分训练步骤无需执行验证阶段的 900 个额外响应生成。基于第一个步骤的时间分解，后续不含验证的训练步骤预计耗时约为第一步骤的 60-70%，即 50-60 分钟。考虑到每 5 步中有 1 步包含验证，平均每个步骤的耗时约为 55 分钟。按此估算，完成 400 个训练步骤大约需要 400 × 55 / 60 ≈ 367 小时，即约 15 天。这个时间虽然较长，但考虑到训练的是 32B 参数模型并且能够达到超越 OpenAI o1-preview 的性能，仍然是可接受的训练成本。
